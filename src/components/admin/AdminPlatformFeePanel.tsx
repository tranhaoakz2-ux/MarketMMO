...
// Panel THẬT "Phí sàn" — giao diện đồng bộ với bản demo đã duyệt
// (AdminDemoPlatformFee.tsx), dùng chung AdminDemoKit. TOÀN BỘ dữ liệu/hành
// vi vẫn THẬT: GET/PATCH /api/admin/platform-fee, POST/DELETE
// /api/admin/platform-fee/schedules[/[id]] — không đổi 1 dòng logic nghiệp
// vụ (% phí sàn áp mọi đơn hàng, đụng tiền + ràng buộc với % hoa hồng). API
...
export default function AdminPlatformFeePanel() {
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader
        title="Phí sàn"
        subtitle="Phí % chung áp trên MỌI đơn hàng của mọi seller (tính trên giá sau giảm giá, freeze lúc đặt đơn). Chỉnh % mặc định, đặt lịch phí theo kỳ, xem tổng phí thu được."
      />
      ...
      <Card padding="p-5">
        ...
        <p className="text-2xl font-black tabular-nums text-[var(--adm-brand)]">{formatVndDemo(dashboard.totalFee)}</p>
        <p className="text-[11px] text-[var(--adm-muted)]">
          từ {dashboard.releasedItems} mục đơn đã giải ngân {from || to ? "(trong kỳ)" : "(toàn thời gian)"}
        </p>
            ...
            <div className="flex flex-col gap-1.5">
              {dashboard.bySeller.map((s) => (
                <div key={s.sellerId} className="flex items-center justify-between text-xs">
                  ...
                  <span className="font-bold tabular-nums text-[var(--adm-text)]">{formatVndDemo(s.totalFee)}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-[360px_1fr]">
        <Card>
          ...
          <div className="flex items-center gap-2">
            <input type="number" step="0.1" value={defaultPct} onChange={(e) => setDefaultPct(e.target.value)} className="w-24 rounded-lg border border-[var(--adm-border)] bg-[var(--adm-surface-2)] px-3 py-2 text-sm text-[var(--adm-text)] outline-none" />
            <span className="text-sm text-[var(--adm-muted)]">%</span>
            <Button variant="primary" disabled={busy} onClick={saveDefault}>
        ...
        <Card>
          ...
          {schedules.length === 0 ? (
            <EmptyState icon={Calendar} title="Chưa có mốc lịch phí nào">Đang dùng % mặc định.</EmptyState>
          ) : (
              ...
              {schedules.map((s) => (
                <div key={s.id} className="flex items-center justify-between gap-2 border-b border-[var(--adm-border)] px-3 py-2 text-xs last:border-0">
                  <div className="min-w-0">
                    ...
                    <span className="ml-2 text-[var(--adm-text)]">
