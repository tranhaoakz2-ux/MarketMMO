...
| **USDT TRC20** (`POST /api/wallet/deposit-usdt`) | Buyer dán TxID, server **xác minh on-chain thật qua TronGrid** (`gettransactioninfobyid`) — kiểm `receipt.result==="SUCCESS"`, quét event log `Transfer` đúng contract USDT-TRC20 + đúng ví sàn, đọc số tiền từ log (không tin calldata). Chống trùng bằng **unique index** trên `gatewayRef`. | **Sẵn sàng hoạt động ngay khi có `usdt_trc20_address` thật** — không phụ thuộc secret bên thứ 3 (TronGrid public endpoint) |
