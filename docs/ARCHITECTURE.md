...
Route gọi song song `releaseDueEscrow({type:"SYSTEM"})`,
`closeDueAuctionSessions()` (mục 10), hoàn tiền pre-order/manual-provision
quá hạn, `expireStaleBankDeposits()` (PENDING→EXPIRED quá 15 phút),
`deleteStaleRejectedDeposits()` (xoá HẲN lệnh nạp EXPIRED/REJECTED đã đủ 24h
— mục 11, KHÔNG bao giờ đụng CONFIRMED), và `sweepAllSellerLevels()`.
...
|---|---|---|
| **VNPay** | Create → redirect có chữ ký HMAC-SHA512 → Return URL (hiển thị cho user) + **IPN** (nguồn chân lý thật để cộng ví, server-to-server, có retry) | **NGỪNG HỖ TRỢ có chủ đích** (2026-08-29) — kill-switch `VNPAY_DISABLED=true` trong `src/lib/payment/vnpay.ts` (đè `isVnpayConfigured()` luôn `false` bất kể cấu hình), chặn thêm ở cả 3 route (`create`/`ipn`/`return`), ẩn hẳn khỏi `/nap-tien`. Code tích hợp vẫn giữ nguyên (không xoá), chỉ cần đổi cờ nếu muốn bật lại. Sàn dùng SePay + USDT. |
| **Ngân hàng qua VietQR + SePay webhook** (`POST /api/wallet/deposit-request` tạo yêu cầu, `POST /api/webhook/sepay` đối soát) | Wizard 3 bước ở `/nap-tien` (`DepositPanel.tsx`): Bước 1 chọn phương thức → Bước 2 nhập số tiền (`MIN_BANK_DEPOSIT_VND`-`MAX_BANK_DEPOSIT_VND`) bấm Xác nhận → **server** (không tin client) sinh `WalletTransaction.depositCode` (UNIQUE, khớp `/NAP[A-Z0-9]{6,}/`) + `expiresAt` (`BANK_DEPOSIT_EXPIRY_MINUTES`, mặc định 15 phút) → Bước 3 hiện QR `img.vietqr.io/image/{BIN}-{STK}-compact.png?amount=...&addInfo={code}` (auto-fill số tiền + nội dung khi quét bằng app ngân hàng) + đếm ngược, trang tự poll `GET /api/wallet/deposit/[id]` mỗi 4s. Webhook SePay xác thực HMAC-SHA256 (`x-sepay-signature`) HOẶC API Key — `timingSafeEqual`, fail-closed 503 nếu thiếu cả 2 secret — khớp `depositCode` trong nội dung CK (14 ngày gần nhất, CHỈ xét dòng còn `status:"PENDING"`) → CONFIRMED, cộng đúng **số tiền THẬT nhận được** (không phải số buyer xin nạp). Khớp SAU KHI `expiresAt` vẫn cộng bình thường (an toàn cho khách), chỉ ghi `adminNote` đánh dấu trễ hạn. Không khớp mã nào → lưu `SepayUnmatchedTransaction` cho admin gán tay. Lệnh EXPIRED (quá 15 phút chưa khớp)/REJECTED (admin từ chối tay) đã KHÔNG còn được webhook nhìn thấy để khớp NGAY từ lúc đổi trạng thái đó (query chỉ xét PENDING) — sau **24h** giữ làm bằng chứng, `deleteStaleRejectedDeposits()` (cron hàng ngày) XOÁ HẲN các dòng này khỏi lịch sử cho gọn; xoá hẳn không đổi gì hành vi khớp webhook (đã không khớp được từ trước đó rồi) — tiền về trễ vẫn tự rơi vào `SepayUnmatchedTransaction` như cũ. TUYỆT ĐỐI không đụng dòng `status:"CONFIRMED"`. | **Chưa hoạt động** — `SEPAY_WEBHOOK_SECRET`/`SEPAY_API_KEY` rỗng, route đang fail-closed (QR/thông tin CK vẫn hiện đúng, chỉ chưa tự động cộng tiền) |
...
| `2026-08-28-member-code.sql` | `User.memberCode` (TEXT, UNIQUE, nullable) + model `MemberCodeCounter` (singleton) — mã thành viên ngắn cho admin tra cứu, gán cố định lúc đăng ký. Ban đầu sinh tuần tự qua `MemberCodeCounter` ("MMO000001"...); từ 2026-08-29 đổi sang sinh NGẪU NHIÊN "MM"+8 số (chống trùng bằng UNIQUE + retry, xem `src/lib/member-code.ts`) — `MemberCodeCounter` không còn được code nào dùng, giữ lại bảng cho khỏi phải DROP TABLE trên Neon |
