...
src/
  ...
  app/
    ...
    trang-ban-hang/san-pham/page.tsx      # render lại ProductVariantManager (dùng chung)
    ...
    api/
      ...
      seller/products/route.ts         # (SELLER) GET — danh sách sản phẩm của chính seller
                                        # kèm variants (dùng lại getMySellerProducts())
  ...
  components/  # Header, NavMegaMenu (dropdown hover Sản phẩm/Dịch vụ/Nạp tiền),
               ...
               # SellerTelegramPanel, SellerVerificationPanel
  context/
...
`src/app/san-pham/[slug]/page.tsx` — 3 khung **luôn hiện đồng thời** (không
phải tab bấm-chuyển-nội-dung, đã đổi theo yêu cầu người dùng), chia đều
`grid-cols-3` (`grid-cols-1` xếp dọc trên mobile), mỗi khung cao bằng nhau và
lấp đầy trọn khung ngoài cùng, có tiêu đề brand riêng + nội dung thật riêng —
...
- **ĐÁNH GIÁ (REVIEWS)**: `RatingStars` + `product.rating`/`reviewCount`
  (số tĩnh từ seed, xem ghi chú ở mục "Domain" cuối file) + link
  `/shop/[seller]` xem đánh giá đầy đủ của gian hàng.
...
- **ProductVariant**: biến thể/gói do seller tự thêm cho sản phẩm CỦA MÌNH
  ...
  tự thêm/xoá, **chưa có** sửa hàng loạt hay tạo sản phẩm mới từ đầu — trang
  này chỉ thêm variant cho sản phẩm đã tồn tại).
...
- [ ] **Sửa biến thể + tạo sản phẩm mới cho seller**: `/quan-ly-san-pham`
      hiện chỉ **thêm/xoá** biến thể (API `PATCH` sửa label/giá/kho đã có
      sẵn ở `seller/products/[productId]/variants/[variantId]/route.ts`
      nhưng **chưa có UI** gọi tới nó — cần thêm form sửa inline). Seller
      cũng **chưa thể tự tạo sản phẩm gốc mới** (category, mô tả, ảnh...) —
      toàn bộ sản phẩm vẫn chỉ tạo qua `prisma/seed.ts`.
