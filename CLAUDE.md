...
/api/seller/categories` + `POST /api/admin/categories/[id]`): nếu sản phẩm
    ...
    xuất — chỉ để admin biết ngữ cảnh khi duyệt (`AdminDashboard.tsx`, mục
    "Danh mục mới chờ duyệt", cùng pattern UI với "Sản phẩm chờ duyệt"),
...
/api/seller/products/[productId]/variants/[variantId]`): `ProductStockItem.
    ...
    → luồng tương tự cho biến thể → xoá variant dọn đúng kho chưa bán, giữ
...
- **Delivery tự động**: đã hiện thực thật qua model `ProductStockItem` — xem
  ...
  phẩm/biến thể CHƯA từng được seller nhập kho thật vẫn hoạt động như cũ
  (đơn hàng chuyển thẳng sang ký quỹ, `OrderItem.deliveredPayload` = null) —
  chỉ sản phẩm/biến thể seller đã bắt đầu nhập kho mới có giao hàng tự động
...
- [x] **Seller tự đề xuất danh mục mới** (nút "+ Thêm danh mục mới chưa có"
      ...
      `AdminDashboard.tsx` (mục "Danh mục mới chờ duyệt"). Khác sản phẩm
